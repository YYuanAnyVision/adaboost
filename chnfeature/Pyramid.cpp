...
void feature_Pyramids::computeChannels(const Mat &image,vector<Mat>& channels) const
{
	...
	//tm3.stop();
	//std::cout << "computeChannels_tm3, ms = " << tm3.getTimeMilli() << std::endl;
...
//	//cv::TickMeter tm1;
//	//tm1.start();
//if (nApprox==0 && pad.width==0 &&pad.height==0)//训练的时候用
//{
//	Mat img_tmp;
//	//compute real 	
...
//		//对每个特征平滑
//		for (int n_chans=0;n_chans<chns.size();n_chans++)
//		{
//			convTri(chns[n_chans],chns[n_chans],Km);	
//		}
//		/*  check the pointer */
//		//float *add1 = (float*)chns[0].data;
//		//for( int c=1;c<chns.size();c++)
//		//{
//		//	//cout<<"pointer "<<static_cast<void*>(approx_chns[c].data)<<" "<< (static_cast<void*>(add1+c*ap_size[ap_id].height*ap_size[ap_id].width))<<endl;
//		//cout<<"pointer "<<(static_cast<void*>(chns[c].data) == (static_cast<void*>(add1+c*((ap_size[s_r].height)*(ap_size[s_r].width))))? true :false)<<endl;
//		//}
//		approxPyramid.push_back(chns);
//	}
//}
//else
//{
...
//			resize(chns_Pyramid[ma][n_chans],py_tmp,py_tmp.size(),0.0,0.0,1);
//			
//			if (nApprox!=0)
//			{
...
////tm1.stop();
////std::cout << "computeChannels_tm1, ms = " << tm1.getTimeMilli() << std::endl;
//}
...
//	{   
//		image= fold[n];
//		feature_set.chnsPyramid(image,Pyramid,scal,scalesh,scalesw);
//		Pyramid_mean.clear();
